...
v <- vect("WBD_HU8_WGS84_export.shp")
v

# Project vector to target CRS (WGS84)
ex <- project( ext(v), from=crs(v), to="+proj=merc +a=6378137 +b=6378137 +lat_ts=0 +lon_0=0 +x_0=0 +y_0=0 +k=1 +units=m +nadgrids=@null +wktext +no_defs")
ex
...
crs(ref) <- "+proj=merc +a=6378137 +b=6378137 +lat_ts=0 +lon_0=0 +x_0=0 +y_0=0 +k=1 +units=m +nadgrids=@null +wktext +no_defs"
...
# This loop reads the shapefile, creates subfolders for each subwatershed (55 total), clips the DEM and Sentinel-2 data by that sub-watershed outline and then returns a raster .tif file for the DEM and a raster .tif file for the Sentinel data.
# There are 55 sub-watersheds 
for(i in 1:55){
  
  ee_roi <- st_read("WBD_HU8_WGS84_export.shp")[i, ] ## read it separately to take the name for the folder - this is the Chesapeake Bay Watershed boundary
  
  # Results folder creation, change ChesTiles/ to your own working directory
  if( !dir.exists(paste0("TestTiles_17_23/", ee_roi$name ) ) ){
    dir.create( paste0("TestTiles_17_23/", ee_roi$name ) )
  }
  
  fn <- paste0("TestTiles_17_23/", ee_roi$name, "/", ee_roi$name, ".tif") # file name template  
...
setwd("C:\\Users\\spacella\\OneDrive - Environmental Protection Agency (EPA)\\SDR Blue Carbon\\Dhond Files\\Blue Carbon Modeling Project\\TestTiles_17_23")
